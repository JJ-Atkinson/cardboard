(ns cardboard.server-components.middleware
  (:require
    [cardboard.server-components.config :refer [config]]
    [cardboard.server-components.pathom :refer [parser]]
  ...
  (html5
    [:html {:lang "en"}
     [:head {:lang "en"}
      ...
      ;[:script {:src "js/vs-monaco/loader.js"}]
      ;[:script "require.config({ paths: { 'vs': 'js/vs-monaco' }});"]
      
      ]
     [:body
      ;[:div#container {:style "height: 200px;"}]
      [:div#app]
